...
class Runtime:
    ...
    def visualize_graph(self, session: Session):
        ...
        for exec_id, execution in session.graph.items():
            if hasattr(execution.node, "__name__"):
                node_name = execution.node.__name__
            elif hasattr(execution.node, "__class__"):
                node_name = execution.node.__class__.__name__
            else:
                node_name = str(execution.node)
